...
do_uninstall() {
    ...
    if [ "$AUTO_CONFIRM" != "true" ]; then
        ...
        echo -e "${YELLOW}检测到免交互强行卸载参数 (-y/--force)，自动跳过二次确认...${NC}"
    fi

    echo -e "\n${YELLOW}[1/4] 停止并清理 Docker 容器与数据卷...${NC}"
    if [ -d "$PROJECT_DIR" ]; then
        cd "$PROJECT_DIR"
        docker compose down -v --remove-orphans --rmi local 2>/dev/null || docker compose down -v --remove-orphans 2>/dev/null || docker compose down -v 2>/dev/null || true
    fi
    docker rm -f ai-ecom-app-1 postgres ai-ecom 2>/dev/null || true
    ...
    echo -e "${GREEN}✓ Nginx 配置清理完成并已重新加载服务${NC}"

    echo -e "\n${YELLOW}[3/4] 检查并清理 Swap 虚拟内存...${NC}"
    if [ -f "/swapfile" ]; then
        CLEAN_SWAP_FLAG=false
        if [ "$AUTO_CONFIRM" = "true" ]; then
            CLEAN_SWAP_FLAG=true
        else
            if [ -c /dev/tty ]; then
                exec < /dev/tty
            fi
    ...
    echo -e "\n${YELLOW}[4/4] 清理配置文件与项目目录...${NC}"
    if [ -d "$PROJECT_DIR" ]; then
        rm -rf "$PROJECT_DIR"
        echo -e "${GREEN}✓ 项目目录 ($PROJECT_DIR) 及其内部数据已完全删除${NC}"
    else
        rm -f .env
        rm -rf public
