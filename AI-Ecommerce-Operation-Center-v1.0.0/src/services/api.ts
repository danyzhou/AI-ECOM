...
export async function testOpenAIConnection(config: { apiKey?: string; model?: string }) {
  const res = await fetch('/api/ai/test/openai', {
    method: 'POST',
    headers: getAuthHeaders(),
...
export async function publishToStoresApi(productId: string, storeIds: string[], productData?: any): Promise<{ success: boolean; results: ProductPublication[]; message: string }> {
  const res = await fetch('/api/publications/publish', {
    method: 'POST',
    headers: getAuthHeaders(),
    body: JSON.stringify({ productId, storeIds, productData }),
  });
  return await res.json();
}

export async function retryWorkflowStep(taskId: string, step: 'chatgpt' | 'gemini' | 'woocommerce') {
...
export async function retryWooCommercePublish(productId: string) {
  const res = await fetch(`/api/woocommerce/retry/${productId}`, {
    method: 'POST',
    headers: getAuthHeaders(),
  });
  const data = await res.json();
  if (!res.ok) {
    throw new Error(data.error || '重新发布失败');
  }
  return data;
}
