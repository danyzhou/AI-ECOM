...
if [ "$EUID" -ne 0 ]; then
  echo -e "${RED}[ERROR] Please run this script as root (e.g., sudo bash scripts/install.sh)${NC}"
  exit 1
fi
...
certbot --nginx -d "${DOMAIN_NAME}" --non-interactive --agree-tos -m "${ADMIN_EMAIL}" --redirect || {
  echo -e "${YELLOW}[WARNING] SSL Certificate acquisition failed or DNS not pointing to server yet.${NC}"
}
