...
import { fetchStores, fetchSystemSettings } from '../services/api';
  ...
  useEffect(() => {
    ...
    const loadStoreData = async () => {
      try {
        ...
        if (isMounted && storeRes && storeRes.success && Array.isArray(storeRes.stores) && storeRes.stores.length > 0) {
          const connected = storeRes.stores.find(s => s.status === 'connected' || s.api_status === 'connected');
          const store = connected || storeRes.stores[0];
          if (store && (store.url || store.wordpress_url)) {
            setActiveStore({
              storeName: store.name || store.store_name || 'WordPress Store',
              siteUrl: store.url || store.wordpress_url || '',
              status: (store.status === 'connected' || store.api_status === 'connected') ? 'connected' : 'disconnected'
            });
            return;
          }
        }
        ...
        if (isMounted && settingsRes && settingsRes.woocommerce && settingsRes.woocommerce.siteUrl) {
          setActiveStore({
            storeName: settingsRes.woocommerce.storeName || 'WordPress Store',
            siteUrl: settingsRes.woocommerce.siteUrl,
            status: settingsRes.woocommerce.status === 'connected' ? 'connected' : 'disconnected'
          });
          return;
        }

        if (isMounted && wcConfig && wcConfig.siteUrl) {
          setActiveStore({
            storeName: wcConfig.storeName || 'WordPress Store',
            siteUrl: wcConfig.siteUrl,
            status: wcConfig.status === 'connected' ? 'connected' : 'disconnected'
          });
          return;
        }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
        ...
        <div className="p-4 bg-slate-900 border border-slate-800 rounded-xl space-y-2">
          ...
          <div className="text-[11px] text-slate-500">AI 智能视觉文案建模</div>
        </div>
        ...
        <div className="p-4 bg-slate-900 border border-slate-800 rounded-xl space-y-2">
          ...
          <div className="flex items-center space-x-1.5 mt-1">
            <span className={`w-2.5 h-2.5 rounded-full ${activeStore?.status === 'connected' ? 'bg-emerald-400 animate-pulse' : activeStore ? 'bg-amber-400' : 'bg-slate-600'}`}></span>
            <span className={`text-sm font-bold ${activeStore?.status === 'connected' ? 'text-emerald-400' : activeStore ? 'text-amber-400' : 'text-slate-400'}`}>
              {activeStore ? (activeStore.status === 'connected' ? '已成功连接' : '未连接 REST') : '未绑定店铺'}
            </span>
          </div>
          <div className="text-[11px] text-slate-500 truncate" title={activeStore?.siteUrl || '未绑定店铺'}>
            {activeStore?.siteUrl || '未绑定店铺'}
          </div>
        </div>
      </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-3">
            ...
            <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 text-xs space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className={`font-medium ${activeStore?.storeName ? 'text-slate-200' : 'text-slate-400'}`}>
                  {activeStore?.storeName || '未绑定店铺'}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-400">站点网址:</span>
                <span className={`font-mono text-[11px] truncate max-w-[160px] ${activeStore?.siteUrl ? 'text-slate-300' : 'text-slate-500'}`}>
                  {activeStore?.siteUrl || '未绑定店铺'}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-400">REST API 状态:</span>
                {activeStore?.status === 'connected' ? (
                  ...
                  <span className="text-slate-500 font-medium">未绑定店铺</span>
                )}
              </div>
            </div>
