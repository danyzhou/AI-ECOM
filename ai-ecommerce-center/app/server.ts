...
app.post("/api/auth/register", (req, res) => {
  ...
  if (usersDb.has(cleanUsername)) {
    return res.status(400).json({ error: "注册失败：该用户名已被注册，请更换用户名" });
  }
  ...
  // Auto-generate session token upon registration
  const token = "ecom_token_" + crypto.randomBytes(24).toString("hex");
